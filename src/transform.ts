...
const DEFAULT_CINEMA_CHAINS = ['cathay', 'gv', 'projector', 'shaw']
      ...
      try {
        ...
        try {
          const module = require(
            path.join('../dist/scripts/transform/', cinemaChain),
          )
          standardiseCinemaChain = module.standardiseCinemaChain
          logger.addBreadcrumb(
            'module_load',
            `Loaded transform module for ${cinemaChain}`,
          )
        } catch (err) {
          logger.error(
            `Error loading transform module for ${cinemaChain}:`,
            err,
          )
          throw err
        } finally {
        ...
        try {
          ...
          logger.trackDataCounts({
            ...
            details: {
              uniqueMovieIds: [
                ...new Set(
                  standardizedData.movies?.map((m: StandardMovie) => m.id) ||
                    [],
                ),
              ].length,
              uniqueCinemaIds: [
                ...new Set(
                  standardizedData.cinemas?.map((c: StandardCinema) => c.id) ||
                    [],
                ),
              ].length,
            },
        ...
        try {
          ...
          logger.addBreadcrumb(
            'file_write',
            `Wrote intermediate data for ${cinemaChain}`,
            {
              path: outputPath,
              size: output.length,
            },
          ...
          logger.error(
            `Error writing intermediate data for ${cinemaChain}:`,
            err,
          )
          throw err
        } finally {
...
function cleanMovieList(data: CinemaData): CinemaData {
  ...
  const span = logger.startTransaction('clean_movie_list', 'transform', {
  ...
  try {
    ...
    logger.trackDataCounts({
      ...
      details: {
        unusedMovies,
        unusedMoviesPercentage:
          ((unusedMovies / movies.length) * 100).toFixed(2) + '%',
