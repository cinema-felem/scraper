...
async function applyUpdates(): Promise<void> {
  ...
  try {
    ...
    try {
      ...
      const result =
        (await removeOrphanedMovies()) as unknown as UpdateOperationResult
    ...
    try {
      ...
      const updateResults =
        (await executeUpdates()) as unknown as UpdateOperationResult
...
async function main(): Promise<void> {
  ...
  const mainTransaction = logger.startTransaction('storage_main', 'storage', {
    steps: steps.join(','),
  })
  ...
  try {
    ...
    if (steps.includes('cinemas')) {
      ...
      try {
        ...
        const result = (await insertCinemas(
          intermediateObject.cinemas,
        )) as unknown as StorageOperationResult
        ...
        logger.trackDataCounts({
          ...
          details: {
            ...
            duplicateRate:
              (
                ((preInsertionCount - insertedCount) / preInsertionCount) *
                100
              ).toFixed(2) + '%',
            updatedCount,
          },
        ...
        logger.info(
          `Inserted ${insertedCount} cinemas, updated ${updatedCount} existing cinemas`,
        )
      } catch (error) {
    ...
    if (steps.includes('movies')) {
      ...
      try {
        ...
        const result = (await insertMovies(
          intermediateObject.movies,
        )) as unknown as StorageOperationResult
        const insertedCount = result?.insertedCount || 0
        ...
        logger.trackDataCounts({
          ...
          details: {
            ...
            skippedDuplicates:
              preInsertionCount - insertedCount - mergedCount - updatedCount,
          },
        })

        logger.info(
          `Inserted ${insertedCount} movies, merged ${mergedCount} duplicates, updated ${updatedCount} existing movies`,
        )
      } catch (error) {
    ...
    if (steps.includes('showtimes')) {
      ...
      try {
        ...
        const uniqueMovieIds = new Set(
          intermediateObject.showtimes?.map((s: any) => s.filmId) || [],
        )
        const uniqueCinemaIds = new Set(
          intermediateObject.showtimes?.map((s: any) => s.cinemaId) || [],
        )

        // Insert showtimes into database
        const result = (await insertShowtimes(
          intermediateObject.showtimes,
        )) as unknown as StorageOperationResult
        const insertedCount = result?.insertedCount || 0
        ...
        logger.info(
          `Inserted ${insertedCount} showtimes, found ${preInsertionCount - insertedCount} duplicates, ${invalidCount} invalid entries`,
        )
      } catch (error) {
