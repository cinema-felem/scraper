...
const mapShowtimes = (showtimeArray: TransformedShowtime[]) => {
  const mappedShowtimeArray = showtimeArray.flatMap(showtime => {
    ...
    const ticketType = cinema.type
    const cinemaParent = cinema?.source?.chainSpecific?.locationCode
    ...
    const { freeValidity, annualPassValidity, eDealsValidity, mPassMovie } =
      movie.source.chainSpecific
    const filmParent = movie.source.chainSpecific.formatGroupParentId
    return {
      id: `gv:${id}`,
...
const mapCinemas = (cinemasArray: GvRawCinema[]): MappedCinema[] => {
...
const cleanupCinemas = (cinemasArray: MappedCinema[]): MappedCinema[] => {
  cinemasArray = cinemasArray.filter(cinema => {
    return cinema?.source?.chainSpecific?.locationCode === cinema?.source?.id
  })
  return cinemasArray
}

const cleanupMovies = (moviesArray: MappedMovies[]): MappedMovies[] => {
  moviesArray = moviesArray.filter(movie => {
    return (
      movie?.source?.chainSpecific?.formatGroupParentId === movie?.source?.id
    )
  })
  return moviesArray
}
...
const standardiseCinemaChain = (cinemaChainObject: {
  ...
  const mappedCinemas: MappedCinema[] = mapCinemas(cinemas)
